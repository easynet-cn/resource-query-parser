...
/**
 * An abstract TokenFilter to make it easier to build graph token filters
 * requiring some lookahead. This class handles the details of buffering up
 * tokens, recording them by position, restoring them, providing access to them,
 * etc.
 */
...
public abstract class LookaheadTokenFilter<T extends LookaheadTokenFilter.Position> extends TokenFilter {
	...
	/**
	 * Call this only from within afterPosition, to insert a new token. After
	 * calling this you should set any necessary token you need.
	 */
	...
	/**
	 * This is called when all input tokens leaving a given position have been
	 * returned. Override this and call insertToken and then set whichever
	 * token's attributes you want, if you want to inject a token starting from
	 * this position.
	 */
	...
	protected boolean peekToken() throws IOException {
		if (DEBUG) {
			System.out.println(
					"LTF.peekToken inputPos=" + inputPos + " outputPos=" + outputPos + " tokenPending=" + tokenPending);
		}
		assert !end;
		...
		if (gotToken) {
			...
			if (startPosData.startOffset == -1) {
				...
				assert startPosData.startOffset == startOffset : "prev startOffset=" + startPosData.startOffset
						+ " vs new startOffset=" + startOffset + " inputPos=" + inputPos;
			}
			...
			if (endPosData.endOffset == -1) {
				...
				// Make sure our input isn't messing up offsets:
				assert endPosData.endOffset == endOffset : "prev endOffset=" + endPosData.endOffset
						+ " vs new endOffset=" + endOffset + " inputPos=" + inputPos;
			}
	...
	/**
	 * Call this when you are done looking ahead; it will set the next token to
	 * return. Return the boolean back to the caller.
	 */
	protected boolean nextToken() throws IOException {
		// System.out.println(" nextToken: tokenPending=" + tokenPending);
		if (DEBUG) {
			System.out.println(
					"LTF.nextToken inputPos=" + inputPos + " outputPos=" + outputPos + " tokenPending=" + tokenPending);
		}
	...
	private boolean insertedTokenConsistent() {
		...
		assert offsetAtt.endOffset() == endPosData.endOffset : "offsetAtt.endOffset=" + offsetAtt.endOffset()
				+ " vs expected=" + endPosData.endOffset;
		return true;
	}
