 ...
 * TokenFilter, its a great idea to test it wrapping this tokenizer instead for
 ...
 * These checks can be disabled with {@link #setEnableChecks(boolean)}.
...
public class MockTokenizer extends Tokenizer {
	...
	private static enum State {
		SETREADER, // consumer set a reader input either via ctor or via
					// reset(Reader)
	...
	public MockTokenizer(Reader input, CharacterRunAutomaton runAutomaton,
			boolean lowerCase, int maxTokenLength) {
		this(AttributeFactory.DEFAULT_ATTRIBUTE_FACTORY, input, runAutomaton,
	...
	/**
	 * Calls {@link #MockTokenizer(Reader, CharacterRunAutomaton, boolean)
	...
	/**
	 * Calls
	 * {@link #MockTokenizer(org.apache.lucene.util.AttributeSource.AttributeFactory,Reader,CharacterRunAutomaton,boolean)
	...
	public final boolean incrementToken() throws IOException {
		assert !enableChecks
				|| (streamState == State.RESET || streamState == State.INCREMENT) : "incrementToken() called while in wrong state: "
				+ streamState;
		clearAttributes();
		for (;;) {
			int startOffset;
			...
			if (cp < 0) {
				...
				if (termAtt.length() < maxTokenLength) {
					// buffer up, in case the "rejected" char can start a new
					// word of its own
				...
				if (state == -1 || runAutomaton.isAccept(state)) {
					// either we hit a reject state (longest match), or
					// end-of-text, but in an accept state
	...
	protected int readCodePoint() throws IOException {
		...
		if (ch < 0) {
			...
			assert !Character.isLowSurrogate((char) ch) : "unpaired low surrogate: "
			...
			if (Character.isHighSurrogate((char) ch)) {
				...
				if (ch2 >= 0) {
					off++;
					assert Character.isLowSurrogate((char) ch2) : "unpaired high surrogate: "
							+ Integer.toHexString(ch)
							+ ", followed by: "
							+ Integer.toHexString(ch2);
					return Character.toCodePoint((char) ch, (char) ch2);
				} else {
					assert false : "stream ends with unpaired high surrogate: "
							+ Integer.toHexString(ch);
				}
			}
			return ch;
	...
	public void close() throws IOException {
		...
		// ignore this
		assert !enableChecks || streamState == State.END
				|| streamState == State.CLOSE : "close() called in wrong state: "
				+ streamState;
		streamState = State.CLOSE;
	}

	boolean setReaderTestPoint() {
		assert !enableChecks || streamState == State.CLOSE : "setReader() called in wrong state: "
				+ streamState;
	...
	public void end() throws IOException {
		...
		// these tests should disable this check (in general you should consume
		// the entire stream)
		try {
