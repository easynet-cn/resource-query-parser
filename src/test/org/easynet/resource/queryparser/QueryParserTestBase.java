...
public abstract class QueryParserTestBase extends Assert {
	...
	public abstract void setAutoGeneratePhraseQueries(QueryParser cqpC,
			boolean value);

	public abstract void setDateResolution(QueryParser cqpC,
			CharSequence field, DateTools.Resolution value);

	public abstract Query getQuery(String query, QueryParser cqpC)
			throws Exception;
	...
	public void assertQueryEquals(QueryParser cqpC, String field, String query,
	...
	public void testSimple() throws Exception {
		...
		assertQueryEquals("türm term term", new MockAnalyzer(random()),
				"türm term term");
	...
	public void testDateRange() throws Exception {
		...
		// we use the default Locale/TZ since LuceneTestCase randomizes it
		Calendar endDateExpected = new GregorianCalendar(TimeZone.getDefault(),
				Locale.getDefault());
	...
	public void assertDateRangeQueryEquals(QueryParser cqpC, String field,
	...
	public void testEscaped() throws Exception {
		...
		assertParseException("XY\\"); // there must be a character after the
										// escape char
	...
	public void testBoost() throws Exception {
		...
		Analyzer a2 = new MockAnalyzer(random(), MockTokenizer.SIMPLE, true,
	...
	public void testBooleanQuery() throws Exception {
		...
		Analyzer purWhitespaceAnalyzer = new MockAnalyzer(random(),
				MockTokenizer.WHITESPACE, false);
	...
	public void testRegexps() throws Exception {
		...
		complex.add(new RegexpQuery(new Term("field", "[a-z]\\/[123]")),
				Occur.MUST);
		complex.add(new TermQuery(new Term("path", "/etc/init.d/")), Occur.MUST);
		complex.add(new TermQuery(new Term("field", "/etc/init[.]d/lucene/")),
				Occur.SHOULD);
		assertEquals(
				complex,
				getQuery(
						"/[a-z]\\/[123]/ AND path:\"/etc/init.d/\" OR \"/etc\\/init\\[.\\]d/lucene/\" ",
						qp));
		...
		assertEquals(new TermQuery(new Term("field", "/boo/")),
				getQuery("\"/boo/\"", qp));
		assertEquals(new TermQuery(new Term("field", "/boo/")),
				getQuery("\\/boo\\/", qp));
	...
	public void testStopwords() throws Exception {
		CharacterRunAutomaton stopSet = new CharacterRunAutomaton(new RegExp(
				"the|foo").toAutomaton());
		QueryParser qp = getParserConfig(new MockAnalyzer(random(),
				MockTokenizer.SIMPLE, true, stopSet));
	...
	public void testPositionIncrement() throws Exception {
		...
		for (int i = 0; i < t.length; i++) {
			...
			assertEquals("term " + i + " = " + t[i]
					+ " has wrong term-position!", expectedPositions[i], pos[i]);
		}
	}
	...
	public void testCollatedRange() throws Exception {
		...
		Query expected = TermRangeQuery.newStringRange(getDefaultField(),
				"collatedabc", "collateddef", true, true);
		Query actual = getQuery("[abc TO def]", qp);
		assertEquals(expected, actual);
	}

	public void testDistanceAsEditsParsing() throws Exception {
		FuzzyQuery q = (FuzzyQuery) getQuery("foobar~2", new MockAnalyzer(
				random()));
	...
	public void testPhraseQueryToString() throws Exception {
		...
		QueryParser qp = getParserConfig(analyzer);
		qp.setEnablePositionIncrements(true);
		PhraseQuery q = (PhraseQuery) getQuery("\"this hi this is a test is\"",
				qp);
	...
	public void testPhraseQueryPositionIncrements() throws Exception {
		...
		QueryParser qp = getParserConfig(new MockAnalyzer(random(),
				MockTokenizer.WHITESPACE, false, stopStopList));

		qp = getParserConfig(new MockAnalyzer(random(),
				MockTokenizer.WHITESPACE, false, stopStopList));
		qp.setEnablePositionIncrements(true);
	...
	public void testMatchAllQueryParsing() throws Exception {
		...
		assertEquals(new MatchAllDocsQuery(),
				getQuery(new MatchAllDocsQuery().toString(), qp));
	...
	public void testNestedAndClausesFoo() throws Exception {
		...
		bq.add(TermRangeQuery.newStringRange("field1", "1", null, true, true),
				BooleanClause.Occur.MUST);
		bq.add(TermRangeQuery.newStringRange("field1", null, "2", true, true),
				BooleanClause.Occur.MUST);
